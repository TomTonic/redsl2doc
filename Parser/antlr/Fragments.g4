...
// -----------------------------------
// URI parsing
// based on https://github.com/antlr/grammars-v4/blob/master/url/url.g4 (BSD-License)

fragment URI            : URIScheme '://' URILogin? URIHost (':' URIPort)? ('/' URIPath)? URIQuery? URIFrag? ;
...
fragment URISearch      : URIString ('=' (URIString | [0-9]+ | URIHex))? ;
fragment URIFrag        : '#' URIString ;
fragment URIString      : ([a-zA-Z~0-9] | URIHex) ([a-zA-Z0-9.-] | URIHex)* ;
fragment URIHex         : ('%' [a-fA-F0-9] [a-fA-F0-9]) + ;

// -----------------------------------
// Unicode

fragment UnicodeCP      : '\\u' (HexDigit (HexDigit (HexDigit HexDigit?)?)?)? ;
...
//fragment UnicodePunct   : [\p{Punctuation}] ;
//fragment UnicodePSep    : [\p{Paragraph_Separator}] ;
//fragment UnicodeLSep    : [\p{Line_Separator}] ;
//fragment UnicodeSpace   : [\p{Space_Separator}] ;
